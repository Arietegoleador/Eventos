    ...
    <section class="panel">
      ...
      <div class="month-grid">
        ...
        ${recent.map(({y,m})=>{
          ...
          const slots=Array(5-visible.length).fill(null).concat(visible);
          ...
          return `<div class="month-col ${m===now.getMonth()&&y===now.getFullYear()?"current":""}">
            <div class="slots">${slots.map(e=>e?`<div class="slot ${resultClass(e.result)} ${e.type==="L"?"l":""}">${e.type==="L"?"L":""}</div>`:`<div class="slot empty"></div>`).join("")}</div>
